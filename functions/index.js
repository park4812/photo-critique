...
8.5~9.4: 프로 수준. Unsplash/500px 인기 사진, 전시/출판 가능한 완성도. 기술, 구도, 감성 모두 탁월.
...
[채점 앵커 - 반드시 참고하세요]
높은 점수 기준:
- Unsplash/500px 인기 사진 수준 = 종합 9.0 이상
- 잡지/전시 출판 가능한 완성도 = 종합 8.5~9.0
- 구도, 빛, 색감 모두 의도적이고 조화로운 사진 = 각 항목 8.5 이상

중간 점수 기준:
- 기본기는 있으나 한두 가지 아쉬운 점이 있는 사진 = 종합 6.5~7.5
- 잘 찍었지만 프로 수준은 아닌 사진 = 종합 7.0~8.0

낮은 점수 기준:
- 기술적 실수가 눈에 띄는 사진 = 종합 5.0~6.5
- 구도나 노출에 명확한 문제가 있는 사진 = 종합 4.0~5.0

점수 분포를 넓게 사용하세요. 모든 사진에 비슷한 점수를 주면 정확한 평가가 아닙니다.
프로급 사진과 아마추어 사진 사이에 최소 2점 이상 차이가 나야 합니다.
...
[중요 - 채점 캘리브레이션]
당신은 점수를 보수적으로 주는 경향이 있습니다. 다음 원칙을 따르세요:

- 프로 수준의 뛰어난 사진이라면 8.5~9.5 범위가 적절합니다
- 하지만 일반적인 사진에 8점 이상을 주는 것도 과대평가입니다
- 프로급과 아마추어급 사이에 점수 차이가 최소 2점 이상 나야 합니다
- 점수 분포를 넓게 사용하세요 (5점~9.5점 범위를 모두 활용)
`;
